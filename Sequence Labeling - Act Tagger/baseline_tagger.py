...
def get_token_features(pos):
    features = []
    if(pos==None):
        features.append("NO_WORDS")
    else:
        [[features.extend(["TOKEN_"+postag[0],postag[1]])] for postag in pos]
    return features

def speaker_change(prev,curr):
    return not(prev==curr) 

def extract_features1(convo):
    features_list = []
    labels = []
    prevspeaker = convo[0][1] 
    token1 = get_token_features(convo[0][2])
    ...
    for utter in range(1,len(convo)):
        ...
        try:
            ...
            token_features = get_token_features(convo[utter][2])
...
def main():
    ...
    ct=0
    for c in convos:
        myfeatures, mylabels = extract_features1(c)
    ...
    for t in testconvos:
        tfeats, tlabels = extract_features1(t)
        xtest.append(tfeats)
